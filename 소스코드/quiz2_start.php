<?
    ...
    setcookie("count_3", "");
    $count = 1;
    if(isset($_COOKIE['count_2'])){
        $count = $_COOKIE['count_2'];
        $count++;
        ...
        if($count>15){
            ...
            echo("
                <script>
                    location.href='quiz2_print.php';
                </script>
            ");            
        }
        else{
            setcookie("count_2", $count);
        }
    }
    if($count==1){
        setcookie("count_2", $count);
...
<html lang="en">
    ...
    <body>
        ...
        $sql="select * from user where id='$id'";
        $result=mysql_query($sql, $connect);
        $tmp_score=mysql_result($result, 0, 7);
        
        if($_POST['mode']=="next" && $_POST['answer']==$_POST['temp']){
            $tmp_score+=1;
            $sql="update user set score_non=$tmp_score where id='$id'";
            $result=mysql_query($sql, $connect);
        }        
        ?>
